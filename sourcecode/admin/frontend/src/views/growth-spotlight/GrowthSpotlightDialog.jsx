...
import { createGrowthSpotlight, updateGrowthSpotlight } from '@/redux-store/slices/growthSpotlight'

const Transition = forwardRef(function Transition(props, ref) {
  return <Slide direction='up' ref={ref} {...props} />
})

const GrowthSpotlightDialog = ({ open, onClose, mode = 'create', spotlight = null }) => {
  const dispatch = useDispatch()
  const { loading } = useSelector(state => state.growthSpotlight)
    ...
    try {
      if (mode === 'edit' && spotlight?._id) {
        ...
        if (!hasChanges) {
          ...
          await dispatch(updateGrowthSpotlight(updatedFields)).unwrap()
        }
      } else {
        ...
        await dispatch(createGrowthSpotlight(payload)).unwrap()
      }
      ...
      setErrors(prev => ({
        ...prev,
        submit: error || 'Failed to save growth spotlight.'
      }))
    } finally {
    ...
    >
      <DialogTitle>
        <Typography variant='h5' component='span'>
          {mode === 'edit' ? 'Edit Growth Spotlight' : 'Create Growth Spotlight'}
        </Typography>
      ...
      <DialogContent className='flex flex-col gap-4 py-4'>
        <TextField
          ...
          placeholder='Optional title for the spotlight card'
        />
        ...
        <TextField
          ...
          placeholder='Optional description shown on expert app'
        />
          ...
          {!imagePreview && (
            <>
              <input
                ...
                type='file'
                id='growth-spotlight-image'
                onChange={handleImageChange}
                style={{ display: 'none' }}
              />
              <Box className='flex flex-col gap-2'>
                <label htmlFor='growth-spotlight-image' className='w-full'>
...
export default GrowthSpotlightDialog
