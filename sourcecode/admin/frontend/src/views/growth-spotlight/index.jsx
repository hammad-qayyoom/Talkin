...
import {
  deleteGrowthSpotlight,
  fetchGrowthSpotlights,
  setPage,
  setPageSize,
  toggleGrowthSpotlightStatus
} from '@/redux-store/slices/growthSpotlight'

import tableStyles from '@core/styles/table.module.css'

import GrowthSpotlightDialog from './GrowthSpotlightDialog'
...
const GrowthSpotlight = () => {
  ...
  const { items, initialLoading, loading, page, pageSize, total } = useSelector(state => state.growthSpotlight)

  const [openDialog, setOpenDialog] = useState(false)
  const [selectedSpotlight, setSelectedSpotlight] = useState(null)
  const [mode, setMode] = useState('create')
  const [confirmOpen, setConfirmOpen] = useState(false)
  const [confirmType, setConfirmType] = useState('delete-growth-spotlight')

  const urlPage = parseInt(searchParams.get('page') || '1', 10)
  const urlPageSize = parseInt(searchParams.get('pageSize') || '10', 10)

  useEffect(() => {
    dispatch(fetchGrowthSpotlights({ page: urlPage, pageSize: urlPageSize }))
  }, [dispatch, urlPage, urlPageSize])

  const handleOpenDeleteDialog = spotlight => {
    setSelectedSpotlight(spotlight)
    setConfirmType('delete-growth-spotlight')
    setConfirmOpen(true)
  }
  ...
  const handleConfirm = () => {
    ...
    if (confirmType === 'delete-growth-spotlight') {
      dispatch(deleteGrowthSpotlight(selectedSpotlight._id))

      return
    }

    dispatch(toggleGrowthSpotlightStatus(selectedSpotlight._id))
  }
  ...
  return (
    <>
      <Box className='mb-3'>
        <Typography variant='h4'>Growth Spotlight</Typography>
        <Typography variant='body2' color='text.secondary'>
          Upload and manage spotlight images shown on expert home page.
        </Typography>
      </Box>
          ...
          {initialLoading ? (
            ...
            <table className={tableStyles.table}>
              ...
              <tbody>
                ...
                <EmprtyTableRow limit={urlPageSize} data={items} columns={columns} noDataLebel='No spotlight found' />
              </tbody>
            </table>
          )}
        </div>
      ...
      <ConfirmationDialog
        ...
        type={confirmType}
        title={
          confirmType === 'delete-growth-spotlight'
            ? 'Are you sure you want to delete this spotlight image?'
...
export default GrowthSpotlight
